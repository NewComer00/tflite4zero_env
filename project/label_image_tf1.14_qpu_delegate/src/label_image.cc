...
namespace tflite
{
  namespace label_image
  {
                                      ...
                                      const DelegateProviders &delegate_providers)
    {
      ...
      if (s->qpu_delegate)
      {
        auto delegate = TfLiteQPUDelegateCreateUnique(nullptr);
        if (!delegate)
        {
          LOG(INFO) << "QPU delegate init failed.";
        }
        else
        {
          LOG(INFO) << "Use qpu delegate acceleration.";
          delegates.emplace("QPU", std::move(delegate));
        }
      }
    ...
    void display_usage()
    {
      LOG(INFO)
          ...
          << "--qpu_delegate, -d: [0|1] qpu delegate\n" //added
          << "\n";
    }
    ...
    int Main(int argc, char **argv)
    {
      ...
      while (1)
      {
        static struct option long_options[] = {
            ...
            {"qpu_delegate", required_argument, nullptr, 'd'}, //added
            {nullptr, 0, nullptr, 0}};
